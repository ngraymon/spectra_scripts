...
def simple_cc_run(max_order, t_final, nof_steps):
    ...
    model = vIO.extract_excited_state_model_op(path_op, highest_order=1)
    ...
    # ---------------------------------------------------------------------
    # create Hamiltonian
    from project.vibronic_hamiltonian import vibronic_hamiltonian as vib_obj

    hamiltonian = vib_obj(
        model,
        HO_size=40, build_H=False, FC=False,
        comparing_to_test_models=False,
        hamiltonian_truncation_order=max_order,
        Z_truncation_order=3, cc_truncation_order=2,
        force_use_optimize=True
    )

    profile_filename = f"cProfile_{file_name}_{order_dict[max_order]}_tf{int(t_final):}"
    # func_string = f"hamiltonian.rk45_integration(t_final={t_final}, nof_points={nof_steps})"

    # run the code
    hamiltonian.rk45_integration(t_final=t_final, nof_points=nof_steps)

    # make plot and store data
    plot_name = f"{file_name}_{order_dict[max_order]}_tf{int(t_final):}"
    abs_output_path, ecd_output_path = hamiltonian.save_acf_data(file_name=plot_name, output_path=root_directory)
