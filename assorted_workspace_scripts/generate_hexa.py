...
sys.path.insert(0, os.path.abspath("$HOME/projects/2022_tamp_testing/t-amplitudes"))  # TEMPORARY
import project
...
def write_spectrum_plotting_file(configuration, *args):
    ...
    # unpack arguments
    nof_points, root_dir, mctdh_file, cc_file, model_name, pbf, tf = args
    # print(f'{root_dir}/{mctdh_file}.pl')
    # doctor the file name to make it look better in the plot
    # print(f'{root_dir}/{cc_file}.pl')
    ...
    output_file = f'{root_dir}/{configuration}_spectrum_{model_name:s}_{nof_points:d}_{int(tf):d}fs_{tau:d}tau.png'
    ...
    plotting_command = '\n'.join([
        ...
        f"set xr [ {left_eV}: {right_EV}]",
        f"set yr [ {min_y}: {max_y}]",
        f"set title '{plot_title:s} spectrum, n-cos = 1, tau: {tau:d}.0 1, {int(tf):d}fs'",
        f"plot \
            '{root_dir}/{cc_file}.pl' every 6 using 1:3 with linespoints lc 'purple' title 'CC',\
        ",
...
def write_acf_plotting_file(configuration, *args):
    ...
    output_file = f"./ACF_{model_name:s}_{nof_points:d}_{t_final:d}fs.png"
    ...
    plotting_command = '\n'.join([
        ...
        "set xr [ 0.0: 25.0]",
        f"plot \
            '{root_dir}/{cc_file}' us 1:2 lc 'green' title 'CC Real (interpolated)',\
...
if __name__ == "__main__":
    ...
    if True:
        ...
        tf = 25.0
        model_name = "_".join([project_name, "vibronic", "linear"])
        calculation_spec = dir_string.format(model_name, tf)
        root_dir = join(work_root, calculation_spec)

        # try to run autospec on the file
        raw_acf_path = join(root_dir, f"ACF_ABS_CC_{model_name:s}_tf{int(tf):>02d}.txt")
        print(f"{raw_acf_path = }")
        if os.path.isfile(raw_acf_path):
            path_cc_spectrum = f"cc_spectrum_{model_name:s}_{int(tf):>02d}fs"

            # if comparing to MCTDH then interpolate it to have a fixed timestep
            new_cc_acf = modify_acf_file(
                root_dir,
                f"ACF_ABS_CC_{model_name:s}_tf{int(tf):>02d}.txt",
                "empty_path", # path_mctdh_acf
                50.0,
                0.5,
            ) # this makes about 200 ish points rather than 400 ish points
            print(f"{new_cc_acf = }")
            ...
            command = generate_cc_pl(
                ...
                input_filename=new_cc_acf
            )
            os.system(command)

        else:
            print('cant find output')
            sys.exit()

        configuration = "cc"

        # try plotting the spectrum and then the ACF
        plotting_file, output_file = write_spectrum_plotting_file(
            configuration, nof_points, root_dir, None, path_cc_spectrum, model_name, 0, tf
        )
        os.system(f"gnuplot {plotting_file}")
        ...
        plotting_file, output_file = write_acf_plotting_file(
            configuration, nof_points, new_cc_acf,
            None, raw_acf_path, model_name
        )
        os.system(f"gnuplot {plotting_file}")
        shutil.copy(output_file, spectrum_dir)
    ...
    for FC_or_not, coupling_order in it.product(["FC", "vibronic"], ["constant", "linear", "quadratic"]):
        ...
        if True and (FC_or_not != "vibronic" or coupling_order != "linear"):
            continue

        print(f"\n{'-'*45}  {FC_or_not} {coupling_order} {'-'*45}\n")

        model_name = "_".join([project_name, FC_or_not, coupling_order])

        for param_list in it.product(*expression_list):

            new_dir_string = dir_string.format(
                model_name,
                param_list[1] # we only want the tf
            )
            calculation_spec = new_dir_string
            root_dir = join(work_root, calculation_spec)

            path_mctdh_acf = join(model_name, "auto")
            # print("path_mctdh_acf", path_mctdh_acf, "\n")

            path_mctdh_spectrum = f"{configuration}_spectrum_{new_dir_string}"
            ...
            if not os.path.isdir(path_to_mctdh_execution_folder):
                ...
                bf, tf = param_list
                name = f"{FC_or_not}_{coupling_order}_bf{bf}"
            ...
            # if slurm seems done then job might be finished
                ...
                if not os.path.isfile(latest_slurm_file):
                    ...
                    if ('memory limit' in data) or ('killed' in data) or ('error' in data) or ('CANCELLED' in data):
